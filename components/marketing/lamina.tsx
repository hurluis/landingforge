...
function Titular({ children, p }: { children: React.ReactNode; p: Paleta }) {
  return (
    <p
      style={{ color: p.texto }}
      className="font-[family-name:var(--font-fraunces)] text-[0.9rem] font-[350] leading-[1.05] tracking-[-0.02em]"
    >
      {children}
    </p>
  );
      ...
      {["Agita el frasco", "Sirve una medida", "Toma en ayunas"].map((t, i) => (
        <div key={i} className="flex items-start gap-2">
          <span
            ...
            className="font-[family-name:var(--font-fraunces)] text-[0.7rem] leading-none"
          >
            {i + 1}
          </span>
